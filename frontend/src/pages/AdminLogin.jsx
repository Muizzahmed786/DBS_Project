        ...
        >
            ...
            >
                ...
                <aside
                    ...
                    style={{ background: 'linear-gradient(135deg, #001f4d 0%, #003f87 100%)' }}
                >
                    ...
                    <div className="flex items-center gap-3 relative z-10">
                        ...
                        <div>
                            ...
                            <p className="text-[0.6875rem] font-medium uppercase tracking-[0.05em] text-white/50">Admin Console</p>
                        </div>
                    </div>
                    ...
                    <div className="relative z-10">
                        <h2 className="text-[clamp(1.8rem,3vw,2.2rem)] font-bold text-white leading-[1.2] tracking-[-0.02em] mb-4">
                            Command the<br />entire<br />system.
                        </h2>
                        <p className="text-[1rem] text-white/70 leading-[1.65] max-w-[280px]">
                            Full administrative access to manage users, challans,
                            RTO offices, and enforcement data.
                        </p>
                        <div className="flex gap-6 mt-8">
                            {[['Full', 'System access'], ['Live', 'Dashboard'], ['Audit', 'Trail']].map(([val, lbl]) => (
                                <div key={lbl} className="flex flex-col">
                                    <span className="text-[1.375rem] font-bold text-white tracking-[-0.02em]">{val}</span>
                                    <span className="text-[0.6875rem] font-medium uppercase tracking-[0.05em] text-white/50 mt-0.5">{lbl}</span>
                                </div>
                            ))}
                        </div>
                    </div>

                    <p className="text-[0.6875rem] text-white/35 tracking-[0.04em] relative z-10">
                ...
                <main className="flex flex-col justify-center gap-8 px-8 md:px-10 py-10 overflow-y-auto bg-white">
                    ...
                    <form onSubmit={handleLogin} className="flex flex-col gap-5">
                        ...
                        <Field id="admin-login-email" label="Admin Email" stagger={1}>
                            <input
                        ...
                        <Field id="admin-login-password" label="Password" stagger={2}>
                            <input
                        ...
                        <button
                            ...
                            style={{
                                ...
                                boxShadow: '0 4px 20px rgba(0,31,77,0.32)',
                                border: 'none',
                                opacity: loading ? 0.75 : 1,
                                cursor: loading ? 'not-allowed' : 'pointer',
                            }}
                            onMouseEnter={(e) => { if (!loading) { e.target.style.transform = 'translateY(-1px)'; e.target.style.boxShadow = '0 8px 28px rgba(0,31,77,0.42)'; } }}
                            onMouseLeave={(e) => { e.target.style.transform = 'translateY(0)'; e.target.style.boxShadow = '0 4px 20px rgba(0,31,77,0.32)'; }}
                            onMouseDown={(e)  => { e.target.style.transform = 'translateY(0)'; e.target.style.boxShadow = '0 2px 10px rgba(0,31,77,0.24)'; }}
                        >
