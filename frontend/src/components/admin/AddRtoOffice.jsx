...
const INITIAL = { rto_code: "", rto_name: "", state: "", district: "", address: "", contact_number: "" };
...
export default function AddRtoOffice() {
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            await addRtoOffice(form);
            setMessage("RTO Office added successfully.");
            setForm(INITIAL);
        } catch (err) {
            setError(err.response?.data?.message || "Failed to add RTO office.");
        } finally {
            setLoading(false);
        }
    };
    ...
    return (
        <div className="min-h-screen" style={{ background: SURFACE }}>
            ...
            <div className="bg-white px-8 py-8"
                ...
                <p className="text-[0.9375rem] mt-1" style={{ color: MUTED }}>Create a new RTO office entry.</p>
            </div>

            {/* ── Form ──────────────────────────────────────── */}
            <div className="px-8 py-8 flex justify-center">
                <form onSubmit={handleSubmit} className="w-full max-w-3xl bg-white rounded-2xl p-8 flex flex-col gap-6"
                    ...
                    <div className="flex items-center gap-3 mb-2">
                        ...
                        <p className="text-[1rem] font-semibold" style={{ color: ON_SURFACE }}>Office Details</p>
                    </div>
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
                        <FormInput label="RTO Code"       name="rto_code"        placeholder="KA-89"         value={form.rto_code}        onChange={handleChange} />
                        <FormInput label="RTO Name"       name="rto_name"        placeholder="RTO Office 2"  value={form.rto_name}        onChange={handleChange} />
                        <FormInput label="State"          name="state"           placeholder="Karnataka"     value={form.state}           onChange={handleChange} />
                        <FormInput label="District"       name="district"        placeholder="Udupi"         value={form.district}        onChange={handleChange} />
                        <FormInput label="Contact Number" name="contact_number"  placeholder="9898784587"    value={form.contact_number}  onChange={handleChange} />
                    </div>
                    ...
                    <div className="flex flex-col">
                        ...
                        <textarea
                            ...
                            className="rounded-xl px-[1rem] py-[0.875rem] text-[0.9375rem] outline-none transition-all duration-200 resize-none"
                            style={{ background: "#d8dde5", border: "none", color: ON_SURFACE }}
                            onFocus={(e) => { e.target.style.background = "#e0e4ea"; e.target.style.boxShadow = `0 0 0 2px ${PRIMARY}`; }}
                            onBlur={(e)  => { e.target.style.background = "#d8dde5"; e.target.style.boxShadow = "none"; }}
                        />
