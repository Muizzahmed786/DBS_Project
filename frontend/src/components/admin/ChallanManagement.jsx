...
const STATUS_TABS = [
  { key: "all",     label: "All Challans", icon: "📋", color: "border-slate-400 text-slate-200",   badge: "bg-slate-700/60 text-slate-300" },
  { key: "pending", label: "Pending",      icon: "⏳", color: "border-amber-500 text-amber-400",   badge: "bg-amber-500/15 text-amber-400" },
  { key: "paid",    label: "Paid",         icon: "✅", color: "border-emerald-500 text-emerald-400", badge: "bg-emerald-500/15 text-emerald-400" },
];
...
const EmptyState = ({ activeKey }) => (
  <tr>
    <td colSpan={11} className="py-20 text-center">
      ...
      <p className="text-slate-400 font-medium">No challans found</p>
    </td>
  </tr>
);
...
const SortTh = ({ label, col, sortCol, sortDir, onSort, noSort, className = "" }) => (
  <th
    ...
    className={`px-4 py-3 text-left text-xs font-semibold text-slate-500 uppercase tracking-wider whitespace-nowrap
      ${!noSort ? "cursor-pointer hover:text-slate-200 select-none" : ""} ${className}`}
  >
    {label}
    {!noSort && (
...
const StatsBar = ({ all, pending, paid }) => {
  ...
  const stats = [
    { label: "Total Challans",  value: all.length     || "—", sub: fmtAmount(totalAmount),   color: "text-white",      bg: "bg-slate-800/50 border-slate-700/60" },
    { label: "Pending",         value: pending.length || "—", sub: fmtAmount(pendingAmount), color: "text-amber-400",  bg: "bg-amber-500/10 border-amber-500/25" },
    { label: "Paid",            value: paid.length    || "—", sub: fmtAmount(paidAmount),    color: "text-emerald-400",bg: "bg-emerald-500/10 border-emerald-500/25" },
    {
      ...
      <div className="bg-slate-900 border-b border-slate-800 px-8 pt-8 pb-0">
        <div className="flex flex-wrap items-start justify-between gap-4 mb-5">
          <div>
            ...
            <h1 className="text-2xl font-bold text-white tracking-tight">Challan Management</h1>
            <p className="text-sm text-slate-400 mt-1">View and monitor all issued challans and their payment status.</p>
          </div>
            ...
            <table className="w-full text-sm">
              ...
              <tbody>
                {loading[activeTab]
                  ...
                  : sorted.map((r, i) => (
                    <tr key={r.challan_id || i} className="border-b border-slate-700/40 hover:bg-slate-700/30 transition-colors">
                      ...
                      <td className="px-4 py-3">
                        <div className="text-sm font-medium text-slate-200 whitespace-nowrap">{r.offender_name || "—"}</div>
                        <div className="text-xs text-slate-500 font-mono mt-0.5">{r.mobile_number || ""}</div>
                      </td>

                      {/* Vehicle */}
                      <td className="px-4 py-3">
                        <div className="font-mono text-xs font-bold text-slate-300 whitespace-nowrap">{r.vehicle_number || "—"}</div>
                        <div className="text-xs text-slate-500 mt-0.5 capitalize">{r.vehicle_class} {r.model ? `· ${r.model}` : ""}</div>
                      </td>
                      ...
                      <td className="px-4 py-3 max-w-[200px]">
                        <div className="text-xs text-slate-300 leading-relaxed line-clamp-2">{r.violation || "—"}</div>
                      </td>

                      {/* Section */}
                      <td className="px-4 py-3">
                        <span className="font-mono text-xs bg-slate-700/60 text-slate-300 px-2 py-0.5 rounded whitespace-nowrap">
                          {r.offence_section || "—"}
                        </span>
                      </td>
          ...
          {!loading[activeTab] && sorted.length > 0 && (
            <div className="px-4 py-3 border-t border-slate-700/60 bg-slate-900/40 flex items-center justify-between flex-wrap gap-2">
              <span className="text-xs text-slate-500">
                ...
                <span className="font-semibold text-slate-300">{rows.length}</span> challans
              </span>
