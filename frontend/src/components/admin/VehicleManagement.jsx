  ...
  return (
    <>
      ...
      <TableWrap>
        ...
        <tbody>
          {loading ? Array.from({ length: 6 }).map((_, i) => <SkeletonRow key={i} cols={cols.length} />)
            : sorted.length === 0 ? <EmptyState icon="🚗" label="vehicles" />
            : sorted.map((r, i) => (
  ...
  return (
    <>
      ...
      <TableWrap>
        ...
        <tbody>
          {loading ? Array.from({ length: 6 }).map((_, i) => <SkeletonRow key={i} cols={cols.length} />)
            : sorted.length === 0 ? <EmptyState icon="📋" label="ownership records" />
            : sorted.map((r, i) => (
              <tr key={r.ownership_id || i} className="border-b border-slate-50 hover:bg-slate-50 transition-colors">
                ...
                <td className="px-4 py-3 text-xs font-mono text-slate-500">{r.ownership_end_date ? fmtDate(r.ownership_end_date) : <span className="text-emerald-600 font-semibold">Current</span>}</td>
                <td className="px-4 py-3"><DateCell value={r.registration_valid_till} /></td>
                <td className="px-4 py-3"><DateCell value={r.insurance_valid_till} /></td>
                <td className="px-4 py-3 font-mono text-xs text-slate-500">
                  {showAadhaar ? (r.aadhaar_number || "—") : `•••• •••• ${String(r.aadhaar_number || "").slice(-4) || "????"}` }
                </td>
              </tr>
            ))}
        </tbody>
...
export default function VehicleManagement() {
  ...
  // ── Fetch all-vehicles & all-ownership on mount ──
